...
impl PulseDevice {
    ...
    fn update_cache(
        ...
        context.borrow().introspect().get_server_info(move |si| {
            if let Some(ref def) = si.default_sink_name {
                cache.borrow_mut().sink = def.to_owned().to_string();
